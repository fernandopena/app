...
export async function saveDiagnosticLocally(answers, result, location, onSucceed) {
  if (isWeb) {
    const diagnostics = await getLocalDiagnostics();
    const now = new Date().getTime();
    const item = JSON.stringify([ ...diagnostics, {
        answers: answers,
        result,
        location,
        created_at: now,
      ...
      db.transaction(
        tx => {
          tx.executeSql(
            ...
            [JSON.stringify(answers), result, JSON.stringify(location)], (tx, result) => {
...
export async function saveLocationLocally(location) {
  return new Promise((resolve, reject) => {
    db.transaction(
      tx => {
        tx.executeSql(
          'INSERT INTO locations (location, created_at) values (?, strftime("%s","now"))',[JSON.stringify(location)], (tx, result) => {
...
export async function getLocalLocations(location) {
  return new Promise((resolve, reject) => {
    db.transaction(
      tx => {
        tx.executeSql('SELECT * FROM locations', [], (tx, result) => {
          let locations = []
...
export async function getLocalDiagnostics() {
  if (isWeb) {
    const diagnostics = JSON.parse(await AsyncStorage.getItem('diagnostics')) || [];
    return diagnostics;
  } else {
    return new Promise((resolve, reject) => {
      db.readTransaction(
        tx => {
          tx.executeSql('SELECT * FROM diagnostics', [], (tx, result) => {
            let diagnostics = []
            const rows = <any>result.rows;
            if (result && rows && rows._array) {
...
export async function deleteLocalDiagnostics() {
  if (isWeb) {
    ...
    return new Promise((resolve, reject) => {
      db.transaction(
        tx => {
          tx.executeSql('DELETE FROM diagnostics', [], (tx, result) => {
